...
export const SURVEY_URL = "https://forms.cloud.microsoft/Pages/ResponsePage.aspx?id=DQSIkWdsW0yxEjajBLZtrQAAAAAAAAAAAAMAABRHK-pUOU4xVk5LT0cxU1dOS0xDRkdPMFdVTTUwVi4u";
...
const state = {
  pid: null, session: null, variant: null, ui_order: null, device: null,
  startHR: 0, start_ts: "", end_ts: "", duration_ms: 0,
  add_ok: 0, add_err: 0, edit_ok: 0, edit_err: 0, delete_ok: 0, delete_err: 0,
  total_errors: 0, notes: ""
};
...
export function markEvent(type, extraNote) {
  switch (type) {
  ...
  if (extraNote) state.notes = state.notes ? `${state.notes} | ${extraNote}` : extraNote;
}
...
export async function completeTask() {
  ...
  const payload = {
    pid: state.pid, session: state.session, variant: state.variant, ui_order: state.ui_order, device: state.device,
    start_ts: state.start_ts, end_ts: state.end_ts, duration_ms: state.duration_ms,
    add_ok: state.add_ok, add_err: state.add_err, edit_ok: state.edit_ok, edit_err: state.edit_err,
    delete_ok: state.delete_ok, delete_err: state.delete_err,
    total_errors: state.total_errors, notes: state.notes, ua: navigator.userAgent
  };
  ...
  try {
    ...
    console.error("[study] send failed", e);
  }
  return payload;
}
